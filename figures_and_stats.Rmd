...
Good_18S_data <- All_18S_data %>% 
  group_by(Library_ID) %>% 
  filter(sum(Abundance)>8000) %>% 
  ungroup()
```

## Check library size ect TODO
...
### Sample-wise rarefaction
```{r}

rarefaction_s <- function(df, marker) {

  accum <- df %>% 
    # Turn data into community df
    group_by(Taxa, Sample) %>% 
    summarise(Abundance = sum(Abundance)) %>% 
    pivot_wider(names_from = "Taxa", values_from = "Abundance") %>%
    column_to_rownames("Sample") %>% 
    # Use Vegan to fit accumulation curve
    specaccum() %>% 
    fitspecaccum(model = "arrhenius")
  
  # Extract model output
  accum_df <- data.frame(Sites=accum$sites, Richness=accum$richness, SD=accum$sd) %>% 
    mutate(Marker = marker)
}

Net_COI_rs <- Net_COI_2017_index_species %>%
  rarefaction_s("Net COI")

Water_COI_rs <- eDNA_COI_2017_index_species %>%
  rarefaction_s("Water COI")

Net_18S_rs <- Net_18S_2017_index_species %>%
  rarefaction_s("Net 18S")

Water_18S_rs <- eDNA_18S_2017_index_species %>%
  rarefaction_s("Water 18S")

Net_Microscopy_rs <- Net_count_2017_index_species %>%
  rarefaction_s("Net Microscopy")


bind_rows(Net_COI_rs, Water_COI_rs, Net_18S_rs, Water_18S_rs, Net_Microscopy_rs) %>% 
  ggplot() +
  #geom_point(aes(x = Sites, y = Richness, color = Marker)) +
  geom_line(aes(x = Sites, y = Richness, color = Marker)) +
  geom_ribbon(aes(x = Sites, ymin = (Richness-2*SD), ymax=(Richness+2*SD), fill = Marker),alpha=0.2)+
  theme_bw()
...
hill_rarefaction <- function(data, nboot = 10, nsteps = 10) {
...
Water_COI_hill <- eDNA_COI_2017_index_species %>% 
  hill_rarefaction() 

Water_COI_hill$hill_table %>% 
  ggplot() +
  geom_line(aes(reads, D0, color = pcr_id)) +
  theme(legend.position = "none")

Net_COI_hill <- Net_COI_2017_index_species %>% 
  hill_rarefaction() 
...
Water_COI_hill$hill_table %>% 
  rename(Sample = pcr_id) %>% 
  left_join(select(eDNA_COI_2017_index_species,
                   Sample, Depth, Date),
            by = "Sample") %>%
  ggplot() +
  geom_line(aes(reads, D1, group = Sample, color = as.character(month(Date)))) +
  #theme(legend.position = "none") +
  scale_color_discrete(seasonal_colors) +
  facet_grid("Depth")
```
...
bind_rows(mutate(Net_count_2017_index_genus, marker = "Count"),
          mutate(Net_COI_2017_index_genus, marker = "COI_bulk"),
          ungroup(mutate(EtOH_COI_2017_index_genus, marker = "COI_sup"))) %>% 
...
mutate(EtOH_COI_index, marker = "COI_sup") %>% 
  filter(Taxa %in% GenusList) %>% 
  mutate(marker = factor(marker, levels = c("Count", "COI_bulk", "COI_sup"))) %>% 
  ggplot() +
