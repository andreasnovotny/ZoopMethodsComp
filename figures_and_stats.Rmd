...
All_COI_data %>%
  filter(Site_ID == "QU39" | is.na(Line_out_depth) == TRUE) %>%
  filter(year(Sample_date) == 2017 | is.na(Line_out_depth) == TRUE) %>% 
  seq_depth_plot() +
  theme_cowplot(12) +
...
All_18S_data %>% 
  filter(is.na(Line_out_depth) == TRUE) %>%
  filter(is.na(Fastq_ID) == FALSE) %>% 
  filter(Abundance > 0) %>% 
  group_by(Fastq_ID, Sample_type, Species) %>% 
  summarise(Abundance = sum(Abundance)) %>% pivot_wider(names_from = Species, values_from = Abundance) 
  
```
None of the blanks passed the quality control for a successful library during sample processing, but we still sequenced 3 field blanks, and 4 PCR blanks for each marker. None of these sequencing libraries passed the quality filtering for a true sample. However, two of the negative controls for COI, and 1 of the negative controls for 18S had sequences that got annotated to Genus level. In the COI controls sequences from Paracalanus (11), Pseudocalanus (63), Acartia (5), Eucalanus (4584) Nanomia (24) were detected. In the negative controls for 18S, sequences from Calanus (149), Erenna (32), and Metridia (144) were detected. Due to the minimal library sizes resulting from sequencing negative controls, cross-contamination was deemed unlikely to have a significant impact on sequencing results.

```{r}
print("Median COI library size:")
Good_COI_data %>%filter(Site_ID == "QU39" | is.na(Line_out_depth) == TRUE) %>%
  filter(year(Sample_date) == 2017 | is.na(Line_out_depth) == TRUE) %>% 
  group_by(Library_ID) %>% 
...
tmp_18S <- Good_18S_data %>%
  filter(Site_ID == "QU39",
         year(Sample_date) == 2017) %>%
  group_by(Fastq_ID, Hakai_ID, Sample_date, Project_name, Line_out_depth, Sample_type) %>% 
  summarise(Abundance = sum(Abundance)) %>% 
  filter(Sample_type == "Zp_Net") %>% 
  ungroup() %>% 
  select(Sample_date, Abundance) %>% 
...
tmp_mic <- df_count_2017 %>%
  ...
  ungroup() %>% 
  select(Sample_date = Date, Abundance = Biomass) %>% 
...
tmp_18S <- Good_18S_data %>%
  ...
  ungroup() %>% 
  select(Sample_date, Abundance, Line_out_depth) %>% 
  mutate(Marker = "18S_Sequences")
...
bind_rows(tmp_18S, tmp_COI) %>%
  group_by(Sample_date, Line_out_depth, Marker) %>% 
  summarise(Abundance = sum(Abundance, na.rm = TRUE)) %>% 
  pivot_wider(names_from = Marker, values_from = Abundance) %>% 
  arrange(Sample_date, as.numeric(Line_out_depth)) %>%
  mutate(`Sequences COI/18S` = paste(COI_Sequences, `18S_Sequences`, sep = " / ")) %>% 
  select(Sample_date, Line_out_depth, `Sequences COI/18S`) %>% 
  pivot_wider(names_from = Line_out_depth, values_from = `Sequences COI/18S`) %>% 
...
Net_count_2017_index_genus %>%
  ...
  arrange(Abundance) %>% 
  ggplot() +
  geom_point(aes(log(Abundance), count))
