...
The ZOIB R package is used for zero or one inflated B-regressions. It is a bayesian MCMC approach that builds on the JAGS sampler.\
\
...
random = 0, EUID = 1:nrow(d), zero.inflation = FALSE, one.inflation = FALSE, n.iter = 1050, n.thin = 5, n.burn=50)
...
bind_rows(mutate(Net_count_2017_index_genus, marker = "Count"),
 ...
 d <- left_join(
    ...
    suffix = c(".count", ".COI")) %>%
  filter(Taxa %in% GenusList,
         is.na(RA_Index.COI) == FALSE,
         is.na(RA_Index.count) == FALSE,
         RA_Index.COI < 1)
 
 d
 
lm(RA_Index.COI ~ RA_Index.count + Taxa, data = d) %>% 
  summary()
