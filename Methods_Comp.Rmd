...
comp.a <- bind_rows(
  mutate(Net_count_2017_index_genus, marker = "count"),
  mutate(Net_COI_2017_index_genus, marker = "COI"),
  mutate(Net_18S_2017_index_genus, marker = "18S")) %>% 
  filter(Taxa %in% GenusList) %>%
  select(!Depth) %>% 
  ...
  pivot_longer(5:6, names_to = "Gene", values_to = "read") %>% 
  filter(is.na(read)==FALSE,
         index != "Abundance")

comp.a
comp.a %>% 
  ggplot() +
  geom_point(aes(count, read, color = Gene, shape = index)) +
  facet_wrap("Taxa") +
  geom_smooth(aes(count, read, color = Gene, linetype = index), method = lm, se = F) +
...
comp.d %>%
  ...
  geom_smooth(aes(RA.count, RA.COI), method = lm, color = "blue", se=F) +
  geom_point(aes(RA_Index.count, RA_Index.COI), color = "red") +
  geom_smooth(aes(RA_Index.count, RA_Index.COI), method = lm, color = "red", se=F) +
  #facet_wrap(facets = "Taxa") +
  theme_bw() +
  ylim(0, 1) +
  facet_wrap("Taxa") +
...
comp.d %>%
  ggplot() +
  geom_point(aes(RA.count, RA.18S), color = "blue") +
  geom_smooth(aes(RA.count, RA.18S), method = lm, color = "blue", se=F) +
  geom_point(aes(RA_Index.count, RA_Index.COI), color = "red") +
  geom_smooth(aes(RA_Index.count, RA_Index.COI), method = lm, color = "red", se=F) +
  #facet_wrap(facets = "Taxa") +
  theme_bw() +
...
```
```{r}
ggarrange(NMDS_relative_abundance, NMDS_presence_absence, NMDS_Index, common.legend = TRUE)
```
