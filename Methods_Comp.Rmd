...
df_COI %>% 
  ...
  write.csv("Figure_output/COI_2019.csv")
...
## 3.1 Pairwise overlap heat maps:
...
## 3.2 Proportion of counted biomass detected by eDNA
...
### TODO: Color bars for broader taxa \<--------------
...
Next we create a merged dataset for the different merkers:
...
ggsave("Figure_output/Seasonal_Bulk_Genus.pdf", width = 7, height = 10)
```
...
ggsave("Figure_output/Seasonal_Comb_Genus.pdf", width = 7, height = 10)
...
ggsave("Figure_output/Seasonal_Bulk_Species.pdf", width = 7, height = 10)
```
...
# Species level index
eDNA_COI_ALL_index_species <- eDNA_COI_All %>% 
  mutate(Taxa = species,
         Abundance = Abundance,
         Depth = Line_out_depth,
         Date = Sample_date) %>% 
...
eDNA_COI_Gradient <- df_COI %>%
  ...
  mutate(Taxa = genus,
         Abundance = Abundance,
         Depth = Line_out_depth,
         Date = paste(Site_ID, Sample_date, sep = "_")) %>% 
  collapseRAindex()
